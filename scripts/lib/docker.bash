...
# moves to `node:<latest LTS>-alpine`. Only rewritten when the version differs;
# a failed LTS lookup warns and leaves the file untouched (the base image is a
# soft lookup, unlike preflight).
rewrite_dockerfile_base_image() {
  local dockerfile="$1" lts old
  ...
  if [[ "$old" == "$lts" ]]; then
    log "docker: base image already node:$lts-alpine — no-op"
    return 0
  fi
  apply_edit "rewrite node:$old-alpine -> node:$lts-alpine in Dockerfile" \
    replace_all_in_file "$dockerfile" "node:$old-alpine" "node:$lts-alpine"
  log "docker: base image node:$old-alpine -> node:$lts-alpine (all stages)"
}
